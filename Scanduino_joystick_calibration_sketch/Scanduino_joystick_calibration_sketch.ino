//SCANDUINO JOYSTICK CALIBRATION SKETCH

//prints current analogue values of x and y axis on joystick
//used to calibrate joystick's central rest zone as the value varies across hardware
//use values obtained in Scanduino_main_sketch.ino
...
void loop(){
  ...
  lcd.setCursor(0, 1);
  lcd.print("Y value: ");
  if (joyStickreadingY < 10){
    lcd.print (000, DEC); //adds three leading zeros to single digit Step size numbers on the display
  }
  if (joyStickreadingY < 100){
    lcd.print (00, DEC); //adds two leading zeros to double digit Step size numbers on the display
  }
  if (joyStickreadingY < 1000){
    lcd.print (0, DEC); //adds one leading zero to triple digit Step size numbers on the display
  }
  lcd.print (joyStickreadingY  , DEC);
